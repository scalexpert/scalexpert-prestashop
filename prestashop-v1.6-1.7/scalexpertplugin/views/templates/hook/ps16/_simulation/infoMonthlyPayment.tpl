    ...
    <div class="sep-Simulations-solution-infoMonthlyPayment">
        {strip}
            {if !empty($solution.hasFeesOnFirstInstallment)}
                {l s='soit un [1]1er prélèvement[/1] de [2]%1$s[/2]' mod='scalexpertplugin'
                    sprintf=[
                ...
                {/if}
                {l s='puis [1]%1$s prélèvements[/1] de [2]%2$s[/2]' mod='scalexpertplugin'
                    sprintf=[
            ...
            {else}
                {if !empty($isModal)}
                    {l s='soit [1]%1$s prélèvements[/1] de [2]%2$s[/2]' mod='scalexpertplugin'
                        sprintf=[
                ...
                {else}
                    {l s='soit [1]%1$s / mois[/1]' mod='scalexpertplugin'
                        sprintf=[
                            '%amountStep%' => $solution.installments.0.amountFormatted|default:''
                        ]
                        tags=[
